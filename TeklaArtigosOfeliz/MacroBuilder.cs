...
namespace TeklaMacroBuilder
{
    ...
    public sealed class MacroBuilder
    {
        ...
        public MacroBuilder TableValueChange(string dialog, string table, string field, string data)
        {
            return AppendMethodCall("TableValueChange", dialog, table, field, data);
        }
        ...
        /// <returns>Reference to self for fluent interface pattern.</returns>
        public MacroBuilder PopupCallback(string callback, string parameter, string frame, string control)
        {
            return AppendMethodCall("PopupCallback", callback, parameter, frame, control);
        }
        ...
        /// </summary>
        /// <returns>Reference to self for fluent interface pattern.</returns>
        public MacroBuilder InvokeCommand(string command, string parameter)
        {
            return AppendMethodCall("InvokeCommand", command, parameter);
        }
        ...
        public void Run()
        {
            try
            {
                ...
                File.WriteAllText(
                    Path.Combine(MacrosPath.Split(';')[0], Name),
        ...
        /// File name format.
        /// </summary>
        private const string FileNameFormat = "MACRO_BUILDER_{0:00}.cs";
        #endregion
    }
}
